...
const InfoSection = ({ title, description, image, reverse }) => {
  return (
    <div className={`flex flex-col md:flex-row items-center gap-10 mt-5 ${reverse ? 'md:flex-row-reverse' : ''}`}>
      <div className="flex-1 w-full text-left">
        <h2 className="text-2xl font-bold text-black mb-5">{title}</h2>
        <p className="text-sm text-align text-gray-500 leading-relaxed">
          {description}
        </p>
      </div>
